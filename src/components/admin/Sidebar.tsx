...
const Sidebar = ({ status, setShow }: { status: any; setShow: any }) => {
  ...
  return (
    <div
      ...
      className={`${
        status === 0 ? "h-[97vh]" : "h-[100vh]"
      }  w-[220px]  sticky top-3 text-white  py-2 left-3 p-4 ${
        status === 0 ? "rounded-[20px] " : "rounded-r-[20px]"
      }`}
    >
      <div>
        <img
          ...
          {data1.map((item) => {
            return (
              <Link
                ...
                className={`flex cursor-pointer items-center gap-3 p-[6px] pl-2 h-[54px] text-xs ${
                  item.id == active ? "bg-[#1A1F37]" : ""
                }  rounded-[15px]`}
              >
                <span
                  className={`rounded-[40%] p-[6px] ${
