...
const Transaction = ({onClose,setShowtransaction,setShowPayment,setShowProfile}:{onClose: () => void; setShowPayment?: React.Dispatch<React.SetStateAction<number>>,setShowProfile?:any,setShowtransaction?:any}) => {
...
const [transactionDetails,setTransactionDetails]=useState<AllTransaction>()
...
useEffect(() => {
 ...
 const handleTransaction = (cardIndex?:any,data?:AllTransaction) => {
  ...
  if(cardIndex===2){
    ...
    console.log(transactionDetails,'asdasdasdasdasdas')
    }
  }
 };
...
const handleArrow=()=>{
  ...
  if(setShowPayment ){
    ...
    if(setShowtransaction){
      setShowtransaction(1)
      console.log('setShowTransaction set -1')
    }
    else{
      onClose()
    }
...
const copyToClipboard = (text:any) => {
  ...
  return (
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex text-center flex-col gap-1">
                          ...
                          >
                           {item.status==='pending'?'-':'+'} {item.deposit_amount || item.withdraw_amount} <span className="text-[10px] font-light">INR</span>
                          </p>
                          <p className={`text-xs ${
                              item.status === "pending"
                                ? "text-[#FF0202]"
   ...
   <div className="min-h-[85vh]">
      <div className="w-[90%]  relative m-auto">
        <div onClick={()=>handleTransaction(1)} className="absolute  cursor-pointer left-2 ">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="43"
            height="42"
      ...
      <div className="flex flex-col gap-5 w-[88%]  m-auto ">
        ...
        <div>
        <p className={`mt-6  ${transactionDetails?.status==='pending'?'text-red-600':'text-green-600'} text-center font-semibold text-xs`}>{transactionDetails?.type}</p>
       <p className={`text-sm mt-1 text-center ${transactionDetails?.status==='pending'?'text-red-500':'text-[#18FB05]'}  `}>{transactionDetails?.deposit_amount||transactionDetails?.withdraw_amount} <span className=''>INR</span></p>
       </div>
          ...
          <div className={`${theme ?`bg-[${themeChange.light.bg2}]` : `bg-[${themeChange.dark.bg2}]`} shadow-2xl p-3 pb-10 flex flex-col gap-[14px] rounded-[4px]`}>
            <div className="flex justify-between w-[100%]">
              ...
              <p className=" text-sm flex items-center gap-4">
                <span>
                  <GoDotFill color={transactionDetails?.status==='pending'?'red':'green'} fontSize="20px" />
                </span>
